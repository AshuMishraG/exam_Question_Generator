...
# Enhanced prompt template for CUET-style English questions
def generate_prompt(question_type):
    topics = [
        "Reading Comprehension", "Jumbled Sentence", "Phrase Replacement",
        "Sentence Improvement", "Cloze Test", "Fill in the Blanks",
        "Wrong Spelt", "Infinitive, Gerund, Participle",
        "Identify the sentence pattern", "Find out the Error", "Verb",
        "Noun", "Articles", "Voices", "Adverbs", "Direct & Indirect Speech",
        "Subject-Verb Agreement", "Conjunctions", "Tenses", "Phrasal Verbs",
        "Idioms and phrases", "Synonyms & antonyms", "One-word substitution",
        "Onomatopoeia", "Anaphora", "Ellipsis", "Select the correct Voice",
        "Error Spotting", "Repetition", "Apostrophe",
        "British English – American English"
    ]
    topics_str = ", ".join(topics)
    
    return f"""
    You are an AI trained to generate CUET (Common University Entrance Test) English sample questions. 
...
def generate_question(question_type="multiple-choice"):
    try:
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert in generating CUET-style English questions."},
...
def generate_image(prompt):
    try:
        detailed_prompt = (
            f"Create an image that resembles a professionally designed English exam question paper. "
